...
export const fieldLabels = {
	...
	google_script_url: (): string => __('Google Script URL'),
...
export const validationSchema = yup.object({
	...
	p2tg: yup.object({
		...
		excerpt_source: yup
			.mixed<DataShape['p2tg']['excerpt_source']>()
		...
		parse_mode: yup.mixed<DataShape['p2tg']['parse_mode']>().oneOf(['none', 'Markdown', 'MarkdownV2', 'HTML']),
		...
		misc: yup
			...
			.of(
				yup
					.mixed<DataShape['p2tg']['misc'][number]>()
					.oneOf(['disable_web_page_preview', 'disable_notification'])
			),
	}),
	proxy: yup.object({
		proxy_method: yup
			.mixed<ProxyFields['proxy_method']>()
			.oneOf(['google_script', 'php_proxy'], () => getErrorMessage('proxy_method', 'invalid'))
			.nullable(),
